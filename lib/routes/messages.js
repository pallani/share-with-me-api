...
async function getLoans () {
  let loansQuery = await db.ref('loans').once('value')
  let loans = loansQuery.val()
  return _.reverse(_.map(loans, (value, key) => _.assign(value, {firebaseKey: `items/${key}`})))
}
  ...
  await bluebird.each(data.entry, async (e) => {
    ...
    await bluebird.each(e.messaging, async (m) => {
      ...
      if (nlp &&
        ...
        let intent = nlp.entities.intent[0].value
        let items
        switch (intent) {
          case 'intent_what-can-borrow':
            ...
            break
          case 'intent_want-to-borrow':
            items = await getItems()
            let whatToBorrow = nlp.entities.thing || []
            if (whatToBorrow.length === 1) {
              let itemToBorrow = whatToBorrow[0].value
              let item = _.find(items, i => i.name === itemToBorrow)
              if (item) {
                let loans = await getLoans()
                let loansForItem = _.filter(loans, l => l.id === item.id)
                if (!_.isEmpty(loansForItem)) {
                  let lastLoan = loansForItem[0]
                  if (moment(lastLoan.end_date).isAfter(moment())) {
                    let availableOn = moment(lastLoan.end_date).from(moment().startOf('day'))
                    await sendMessage(sender, `Sorry the item is currently on loan.  It will be available ${availableOn}. Do you want me to alert you when it becomes available?`)
                  } else {
                    await sendMessage(sender, `Yes! A ${itemToBorrow} is available for loan. Please come to the CC to borrow.`)
                  }
                } else {
                  await sendMessage(sender, `Yes! A ${itemToBorrow} is available for loan. Please come to the CC to borrow.`)
                }
              } else {
                await sendMessage(sender, `We do not have ${itemToBorrow} for loan. Sorry.`)
              }
            } else {
              await notSure(m)
            }
